module reg_writer(done, regnum, direction, go, clock, reset);
	...
	wire sStart_next = (sGarbage&go | sDone&go | sStart&go) & ~reset;

	wire sUp1_next = sStart&direction&~go & ~reset;//here
	...
	dffe fsGarbage(sGarbage, sGarbage_next, clock, 1'b1, 1'b0);//1'b0
